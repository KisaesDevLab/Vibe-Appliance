...
EOF
  ...
  # ---- VIBE SHIELD (admin API key) ------------------------------------
  # Optional section. The gateway admin key is generated by
  # lib/enable-app.sh's _render_app_env on first enable and persisted to
  # /opt/vibe/env/vibe-shield.env (mode 600). Surface it here so the
  # operator gets a single canonical archive of every credential they
  # need — same posture as the Duplicati keys above. Skipped when the
  # per-app env file doesn't exist yet (Shield not enabled).
  local shield_env="${VIBE_ENV_DIR}/vibe-shield.env"
  local shield_key=""
  if [[ -f "$shield_env" ]]; then
    shield_key="$(awk -F= '$1=="GATEWAY_ADMIN_KEY" { sub(/^[^=]+=/, "", $0); print; exit }' "$shield_env" 2>/dev/null || true)"
  fi
  if [[ -n "$shield_key" ]]; then
    cat >>"$tmp" <<EOF

================================================================
 VIBE SHIELD (Anthropic gateway admin)
================================================================
URL:       ${server_url}/shield/
Auth:      paste the key below into the X-Admin-Key field on the
           admin login form (Shield has no user model — the key IS
           the credential, refresh = re-prompt).

  GATEWAY_ADMIN_KEY: ${shield_key}

Generated at enable time and preserved across re-renders by
lib/enable-app.sh. To rotate, edit /opt/vibe/env/vibe-shield.env
and restart vibe-shield-gateway — no other Vibe state depends on
this value (it gates the admin UI only, not the request-encryption
KEK).
================================================================
EOF
  fi
