...
phase_caddy() {
  ...
  # Host bind address for Caddy's :80 / :443. Domain + LAN modes bind
  # all interfaces (0.0.0.0) — domain needs :80 reachable from the
  # public internet for HTTP-01 ACME challenges, LAN needs it for
  # office-network access. Tailscale mode pins to 127.0.0.1 because
  # `tailscale serve` on the host already proxies tailnet HTTPS into
  # localhost; exposing :80 on the droplet's public IP serves the
  # appliance to anyone who scans the IP, which contradicts the
  # privacy posture operators expect from tailscale mode.
  _write_compose_env_file
  state_set_phase caddy ok
}

# Write /opt/vibe/appliance/.env so docker compose's variable
# substitution in docker-compose.yml's ports directive resolves to the
# right host bind address for the current mode. Auto-loaded by every
# `docker compose ...` invocation against that compose file (the
# console's enable-app.sh, update.sh, and this script's own
# phase_core_up all benefit).
#
# Behaviour:
#   tailscale → 127.0.0.1  (loopback only; tailscaled proxies in)
#   lan       → 0.0.0.0    (LAN reachable)
#   domain    → 0.0.0.0    (HTTP-01 needs public :80)
#
# Backward compat: an existing install whose .env is missing keeps the
# 0.0.0.0 default from docker-compose.yml's `${HOST_BIND_HTTP:-0.0.0.0}`
# fallback. Operators who switch modes via the console settings page
# trigger the same write through console/server.js's
# network-mode-switch handler.
_write_compose_env_file() {
  local bind="0.0.0.0"
  if [[ "$CONFIG_MODE" == "tailscale" ]]; then
    bind="127.0.0.1"
  fi
  local env_path="${APPLIANCE_DIR}/.env"
  log_step "writing compose .env" host_bind="$bind" mode="$CONFIG_MODE"
  cat >"$env_path" <<EOF
# Generated by lib/bootstrap.sh phase_caddy. Do not hand-edit; rerun
# bootstrap or trigger a mode switch from the admin Settings page.
HOST_BIND_HTTP=${bind}
