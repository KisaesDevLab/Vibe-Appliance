...
**As of 2026-07-23**, per-app subdomain routing is available again in domain mode as an **opt-in setting**, done the way that avoids the original breakage. `DOMAIN_ROUTING_MODE` (appliance.env, Settings → Network → "App routing layout") selects the layout:
...
**Caddyfile sketch (domain mode, one host fronting all apps):**
