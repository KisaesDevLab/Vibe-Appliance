...
app.post('/api/v1/admin/network-mode/switch', requireAdmin, testRateLimit, async (req, res) => {
  ...
  // Host bind for Caddy. Tailscale mode pins to 127.0.0.1 so the
  // appliance isn't reachable on plain HTTP at the droplet's public IP
  // (tailscaled proxies tailnet HTTPS into localhost). LAN + domain
  // modes keep 0.0.0.0 — LAN needs it for office traffic, domain needs
  // public :80 for HTTP-01 ACME challenges. Written to
  // /opt/vibe/appliance/.env so docker compose's
  // `${HOST_BIND_HTTP:-0.0.0.0}:80:80` substitution picks it up on
  // every subsequent compose call (this handler, enable-app.sh,
  // update.sh, bootstrap re-runs). Owner is the console's runtime
  // user; permissions 644.
  try {
    const bind = mode === 'tailscale' ? '127.0.0.1' : '0.0.0.0';
    const envPath = path.join(APPLIANCE_DIR, '.env');
    const envBody = [
      '# Generated by /api/v1/admin/network-mode/switch. Do not hand-edit;',
      '# the next bootstrap or mode change will overwrite.',
      `HOST_BIND_HTTP=${bind}`,
      `HOST_BIND_HTTPS=${bind}`,
      '',
    ].join('\n');
    fs.writeFileSync(envPath, envBody, { mode: 0o644 });
    log('info', 'compose .env updated for mode switch', { bind, mode });
  } catch (err) {
    log('warn', 'failed to write compose .env; Caddy port-binding not updated', { err: err.message });
  }
  ...
  if (render.code === 0) {
    ...
    // Recreate Caddy if its port mapping changed. `caddy reload`
    // (above) re-reads the Caddyfile but does NOT recreate the
    // container — so a `${HOST_BIND_HTTP}` change from 0.0.0.0 to
    // 127.0.0.1 (or vice versa) wouldn't take effect until the next
    // operator-triggered restart. `docker compose up -d caddy` diffs
    // desired against actual and recreates only if the port mapping
    // (or any other compose-tracked attribute) changed. No-op in the
    // common case where the bind address didn't move (e.g.,
    // lan↔domain). Brief — Caddy's restart costs ~2-4 seconds; the
    ...
    if (portRecreate.code !== 0) {
      log('warn', 'Caddy compose up -d caddy returned non-zero; port-binding may not have switched', {
        code: portRecreate.code,
        stderr: trim(portRecreate.stderr, 300),
      });
    } else {
      log('info', 'Caddy compose up applied (port bind reconciled)');
    }
...
app.get('/api/v1/first-login', requireAdmin, async (_req, res) => {
  ...
  const items = Object.values(MANIFESTS)
    ...
    .map(m => {
      ...
      // passwordEnvKey: pull the live value from /opt/vibe/env/<slug>.env.
      // Used by apps where the password is generated by the appliance at
      // enable time (vibe-shield's GATEWAY_ADMIN_KEY) rather than baked
      // into the manifest. Empty when the per-app env file doesn't exist
      // yet (app not enabled) or doesn't contain the named key — the UI
      // already handles a missing password as "(not generated yet)".
      let password = fl.password || null;
      if (fl.passwordEnvKey) {
        const perApp = parseEnvFile(path.join(ENV_DIR, m.slug + '.env'));
        const live = perApp[fl.passwordEnvKey];
