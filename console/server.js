...
// On-demand container logs for a given app. Reads the manifest's
// routing to derive container names (same logic as
// lib/enable-app.sh:_app_services — `service:port` upstream entries
// where the service name doubles as the container name). Concatenates
// `docker logs <container> --tail <N>` for every service in the
// manifest, with a divider between containers. Used by the "View
// container logs" button on each app card; useful both during
// failure forensics and ongoing diagnostics on a healthy app.
//
// Returns text/plain so the operator can copy-paste the whole thing
// into chat without escaping. Lines is clamped to 1-2000.
app.get('/api/v1/container-logs/:slug', requireAdmin, async (req, res) => {
  const slug = req.params.slug;
  if (!SLUG_RE.test(slug)) return res.status(400).json({ error: 'invalid slug' });
  const m = MANIFESTS[slug];
  if (!m)                   return res.status(404).json({ error: 'unknown app' });

  let lines = parseInt(req.query.lines, 10);
  if (!Number.isFinite(lines) || lines < 1) lines = 200;
  if (lines > 2000) lines = 2000;

  // Container-name extraction. Same regex as the bash side. `service:port`
  // upstreams; `service` doubles as the container_name. Skip duplicates
  // so multi-subdomain apps (Vibe-Connect) don't dump the same logs twice.
  const upstreamRe = /^([a-z0-9.-]+):\d+$/;
  const containers = [];
  const routing = (m.routing || {});
  const candidates = [routing.default_upstream || ''];
  for (const matcher of (routing.matchers || [])) {
    candidates.push(matcher.upstream || '');
  }
  for (const c of candidates) {
    const mm = c.match(upstreamRe);
    if (mm && !containers.includes(mm[1])) containers.push(mm[1]);
  }

  if (!containers.length) {
    res.type('text/plain; charset=utf-8');
    return res.status(200).send(
      `No container names could be derived from ${slug}'s manifest routing.\n` +
      `Manifest must declare routing.default_upstream as 'name:port'.\n`
    );
  }

  // Run `docker logs` for each container sequentially; concat with banners.
  const out = [];
  for (const name of containers) {
    out.push(`========================================`);
    out.push(`== ${name} (last ${lines} lines)`);
    out.push(`========================================`);
    try {
      const text = await new Promise((resolve) => {
        let buf = '';
        // --tail <N> + --timestamps + 2>&1: docker logs writes container
        // stderr separately, which is exactly the part we usually want
        // (most apps log to stderr for errors). Merge into one stream so
        // the order matches the container's actual output.
        const child = spawn('docker', ['logs', '--tail', String(lines), '--timestamps', name], {
          stdio: ['ignore', 'pipe', 'pipe'],
        });
        child.stdout.on('data', (d) => { buf += d.toString(); });
        child.stderr.on('data', (d) => { buf += d.toString(); });
        const t = setTimeout(() => { try { child.kill('SIGKILL'); } catch (_) {} }, 5000);
        child.on('exit', () => { clearTimeout(t); resolve(buf || '(no output)'); });
        child.on('error', (err) => { clearTimeout(t); resolve(`error: ${err.message}\n`); });
      });
      out.push(text);
    } catch (err) {
